CREATE OR REPLACE FUNCTION GetTeamScore(team VARCHAR, year INT, given_week INT default -1, team2 VARCHAR default null)
...
BEGIN
    ...
    THEN
        RETURN QUERY select home_team, away_team, week, season_year, season_type, home_score, away_score, finished from game where (home_team=team or away_team=team) and season_year=year and season_type<>'Preseason' ORDER by start_time desc limit 1;
    ELSIF given_week <> -1 AND (team2 = '') IS NOT FALSE  -- given a week but no team 2
    THEN
        RETURN QUERY select home_team, away_team, week, season_year, season_type, home_score, away_score, finished from game where (home_team=team or away_team=team) and season_year=year AND week = given_week and season_type<>'Preseason' limit 1;
    ELSIF given_week = -1 AND (team2 = '') IS NOT TRUE  -- Given a team 2 but no week
    THEN
        RETURN QUERY select home_team, away_team, week, season_year, season_type, home_score, away_score, finished from game where ((home_team=team and away_team=team2) or (home_team=team2 and away_team=team)) and season_year=year and season_type<>'Preseason' ORDER by start_time desc limit 1;
